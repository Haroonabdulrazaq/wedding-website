...
try {
  ...
  const rsvpsWithoutBarcode = db
    .prepare('SELECT id FROM rsvp WHERE barcode IS NULL')
    .all();
  for (const rsvp of rsvpsWithoutBarcode) {
    const barcode =
      'HL2026-' + crypto.randomBytes(6).toString('hex').toUpperCase();
    try {
      db.prepare('UPDATE rsvp SET barcode = ? WHERE id = ?').run(
        barcode,
        rsvp.id,
      );
    } catch (e) {
      ...
      const newBarcode =
        'HL2026-' + crypto.randomBytes(8).toString('hex').toUpperCase();
      db.prepare('UPDATE rsvp SET barcode = ? WHERE id = ?').run(
        newBarcode,
        rsvp.id,
      );
  ...
  if (rsvpsWithoutBarcode.length > 0) {
    console.log(
      `Migration: Generated barcodes for ${rsvpsWithoutBarcode.length} existing RSVPs`,
    );
  }
} catch (err) {
  console.log('Migration check:', err.message);
...
const insR = db.prepare(
  'INSERT OR IGNORE INTO reactions(emoji,cnt) VALUES(?,0)',
);
...
const Q = {
  ...
  // Guestbook
  gbAdd: (name, email, loc, rel, msg) =>
    db
      .prepare(
        'INSERT INTO guestbook(name,email,location,relation,message) VALUES(?,?,?,?,?)',
      )
      .run(name, email, loc, rel, msg),
  gbGet: (lim = 30, off = 0) =>
    db
      .prepare(
        'SELECT * FROM guestbook WHERE approved=1 ORDER BY created_at DESC LIMIT ? OFFSET ?',
      )
      .all(lim, off),
  gbCount: () =>
    db.prepare('SELECT COUNT(*) as n FROM guestbook WHERE approved=1').get().n,
  gbAll: () =>
    db.prepare('SELECT * FROM guestbook ORDER BY created_at DESC').all(),
  gbDel: (id) => db.prepare('DELETE FROM guestbook WHERE id=?').run(id),
  gbApprove: (id) =>
    db.prepare('UPDATE guestbook SET approved=1 WHERE id=?').run(id),
  gbHide: (id) =>
    db.prepare('UPDATE guestbook SET approved=0 WHERE id=?').run(id),
  gbResetAll: () => db.prepare('DELETE FROM guestbook').run(),

  // RSVP
  rsvpAdd: (n, e, p, att, guest, sz, diet, msg, barcode) =>
    db
      .prepare(
        'INSERT INTO rsvp(name,email,phone,attendance,guest_of,party_size,dietary,message,barcode) VALUES(?,?,?,?,?,?,?,?,?)',
      )
      .run(n, e, p, att, guest, sz, diet, msg, barcode),
  rsvpAll: () =>
    db.prepare('SELECT * FROM rsvp ORDER BY created_at DESC').all(),
  rsvpStats: () =>
    db
      .prepare(
        'SELECT attendance, COUNT(*) as cnt, SUM(party_size) as ppl FROM rsvp GROUP BY attendance',
      )
      .all(),
  ...
  rsvpGetByBarcode: (barcode) =>
    db.prepare('SELECT * FROM rsvp WHERE barcode=?').get(barcode),
  rsvpMarkBarcodeSent: (id) =>
    db.prepare('UPDATE rsvp SET barcode_sent=1 WHERE id=?').run(id),
  rsvpWithEmails: () =>
    db
      .prepare("SELECT * FROM rsvp WHERE email IS NOT NULL AND email != ''")
      .all(),
  rsvpPhysicalWithEmails: () =>
    db
      .prepare(
        "SELECT * FROM rsvp WHERE email IS NOT NULL AND email != '' AND attendance = 'physical'",
      )
      .all(),

  rsvpFindByName: (name) =>
    db.prepare('SELECT id FROM rsvp WHERE LOWER(name) = LOWER(?)').get(name),
  rsvpResetAll: () => db.prepare('DELETE FROM rsvp').run(),
  resetBarcodeSent: () => db.prepare('UPDATE rsvp SET barcode_sent = 0').run(),
  rsvpMarkScanned: (id) =>
    db
      .prepare('UPDATE rsvp SET scanned_at = CURRENT_TIMESTAMP WHERE id = ?')
      .run(id),

  // Blessings
  blessAdd: (name, loc, msg) =>
    db
      .prepare('INSERT INTO blessings(name,location,message) VALUES(?,?,?)')
      .run(name, loc, msg),
  blessGet: (lim = 40) =>
    db
      .prepare(
        'SELECT * FROM blessings WHERE visible=1 ORDER BY created_at DESC LIMIT ?',
      )
      .all(lim),
  blessAll: () =>
    db.prepare('SELECT * FROM blessings ORDER BY created_at DESC').all(),
  blessDel: (id) => db.prepare('DELETE FROM blessings WHERE id=?').run(id),
  blessResetAll: () => db.prepare('DELETE FROM blessings').run(),

  // Reactions
  reactBump: (emoji) => {
    db.prepare('UPDATE reactions SET cnt=cnt+1 WHERE emoji=?').run(emoji);
    return (
      db.prepare('SELECT cnt FROM reactions WHERE emoji=?').get(emoji)?.cnt || 0
    );
  },
  ...
  viewerJoin: (sid, name) =>
    db
      .prepare('INSERT OR REPLACE INTO viewers(sid,name) VALUES(?,?)')
      .run(sid, name),
  viewerLeave: (sid) =>
    db
      .prepare('UPDATE viewers SET left_at=CURRENT_TIMESTAMP WHERE sid=?')
      .run(sid),
  viewerCount: () =>
    db.prepare('SELECT COUNT(*) as n FROM viewers WHERE left_at IS NULL').get()
      .n,
  viewerNames: () =>
    db
      .prepare(
        'SELECT name FROM viewers WHERE left_at IS NULL ORDER BY joined_at DESC LIMIT 30',
      )
      .all(),

  // Chat
  chatAdd: (name, msg) =>
    db.prepare('INSERT INTO chat(name,message) VALUES(?,?)').run(name, msg),
  chatRecent: (lim = 60) =>
    db
      .prepare('SELECT * FROM chat ORDER BY created_at DESC LIMIT ?')
      .all(lim)
      .reverse(),
