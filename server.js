...
function requireAdmin(req, res, next) {
  ...
  const expected = makeToken(
    ...
    process.env.ADMIN_PASSWORD || 'RayOfLove2026!',
  );
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) =>
    cb(null, uuid() + '-' + file.originalname.replace(/[^a-zA-Z0-9.]/g, '_')),
});
...
app.post('/api/guestbook', postLimit, (req, res) => {
  ...
  if (message.length > 600)
    return res
      .status(400)
      .json({ error: 'Message too long (max 600 characters)' });
  if (name.length > 80)
...
app.post('/api/rsvp', postLimit, (req, res) => {
  ...
  if (now > deadline) {
    return res.status(403).json({
      error:
        'RSVP deadline has passed. RSVP closed on April 17, 2026 at 11:59 PM.',
    });
  }
  ...
  if (!attendance || !['physical', 'virtual', 'regrets'].includes(attendance))
    return res.status(400).json({
      error:
        'Please select how you will attend (In Person, Virtually, or Regrets)',
    });
  if (name.length > 80)
    return res.status(400).json({ error: 'Name too long (max 80 characters)' });
  ...
  if (existing) {
    return res.status(409).json({
      error:
        'An RSVP with this name already exists. If you need to update your RSVP, please contact us via WhatsApp.',
    });
  }

  // Generate unique barcode
  const barcode =
    'HL2026-' + crypto.randomBytes(6).toString('hex').toUpperCase();
...
app.get('/api/rsvp/stats', (req, res) =>
  res.json({ stats: Q.rsvpStats(), total: Q.rsvpCount() }),
);
...
app.post('/api/blessings', postLimit, (req, res) => {
  ...
  if (name.length > 40)
    return res.status(400).json({ error: 'Name too long (max 40 characters)' });
...
app.get('/api/verify/:barcode', (req, res) => {
  ...
  if (!barcode || !barcode.startsWith('HL2026-')) {
    return res
      .status(400)
      .json({ valid: false, error: 'Invalid barcode format' });
  }
...
app.post('/admin/login', loginLimit, (req, res) => {
  ...
  const ep = process.env.ADMIN_PASSWORD || 'RayofLove2026!';
...
app.post('/admin/go-live', requireAdmin, (req, res) => {
  ...
  if (!meetLink)
    return res.status(400).json({ error: 'Please provide a stream link' });
  try {
    new URL(meetLink);
  } catch (e) {
...
app.get('/admin/rsvp', requireAdmin, (req, res) =>
  res.json({ rsvps: Q.rsvpAll(), stats: Q.rsvpStats() }),
);
app.delete('/admin/rsvp/:id', requireAdmin, (req, res) => {
  Q.rsvpDel(+req.params.id);
  res.json({ ok: true });
});
...
app.post('/admin/send-barcodes', requireAdmin, async (req, res) => {
  ...
  if (rsvpsWithEmails.length === 0) {
    return res.json({
      ...
      sent: 0,
      message:
        'No physical attendees with email addresses found. Barcodes are only sent to guests attending in person.',
    });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: 'Nodemailer not installed. Run: npm install nodemailer' });
  }
      ...
      html: `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #0C1A0E; border-radius: 12px; border: 1px solid #B8922C;">
          ...
          <div style="background: rgba(15,122,106,0.2); padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <p style="color: #FDFAF2; margin: 8px 0;"><strong>Date:</strong> Sunday, 25 April 2026</p>
          ...
          <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid rgba(184,146,44,0.3);">
            <p style="color: #3DD4C8; font-size: 0.85rem;">#RayOfLove2026</p>
...
app.get('/admin/guestbook', requireAdmin, (req, res) =>
  res.json({ entries: Q.gbAll() }),
);
app.delete('/admin/guestbook/:id', requireAdmin, (req, res) => {
  Q.gbDel(+req.params.id);
  res.json({ ok: true });
});
app.put('/admin/guestbook/:id/hide', requireAdmin, (req, res) => {
  Q.gbHide(+req.params.id);
  res.json({ ok: true });
});
app.put('/admin/guestbook/:id/approve', requireAdmin, (req, res) => {
  Q.gbApprove(+req.params.id);
  res.json({ ok: true });
});

app.get('/admin/blessings', requireAdmin, (req, res) =>
  res.json({ blessings: Q.blessAll() }),
);
app.delete('/admin/blessings/:id', requireAdmin, (req, res) => {
  Q.blessDel(+req.params.id);
  res.json({ ok: true });
});
...
app.post('/admin/reset-data', requireAdmin, (req, res) => {
  ...
  try {
    if (type === 'rsvp') Q.rsvpResetAll();
      ...
      return res
        .status(400)
        .json({ error: 'Invalid type. Use: rsvp, guestbook, or blessings' });
    res.json({ ok: true });
  } catch (e) {
...
app.get('/import-rsvp-data', (req, res) =>
  res.sendFile(path.join(__dirname, 'public/import-rsvp-data.html')),
);
...
io.on('connection', (socket) => {
  ...
  socket.on('chat:send', (data) => {
    ...
    const now = Date.now(),
      last = chatRateMap.get(socket.id) || 0;
...
async function sendScheduledEmails(emailNum) {
  ...
  if (rsvpsWithEmails.length === 0) {
    console.log(
      `Scheduled email ${emailNum}: No physical attendees with emails`,
    );
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #0C1A0E; border-radius: 12px; border: 1px solid #B8922C;">
          ...
          <div style="background: rgba(255,255,255,0.05); padding: 25px; border-radius: 8px; margin-bottom: 20px;">
            <h2 style="color: #EFC060; font-size: 1.1rem; margin-top: 0;">Dear ${rsvp.name},</h2>
            <p style="color: #FDFAF2; line-height: 1.8;">${
              isFirstEmail
                ? 'Assalamu alaikum! Thank you for your RSVP. Your unique QR code for the wedding celebration is below:'
                : 'This is a friendly reminder that the wedding is tomorrow! Please remember to bring your QR code for entry:'
            }</p>
          ...
          <div style="background: rgba(15,122,106,0.2); padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            <h3 style="color: #EFC060; font-size: 1rem; margin-top: 0;">📍 Event Details</h3>
            <p style="color: #FDFAF2; margin: 8px 0;"><strong>Date:</strong> Saturday, 25 April 2026</p>
            <p style="color: #FDFAF2; margin: 8px 0;"><strong>Nikkah:</strong> 10:00 AM WAT</p>
            <p style="color: #FDFAF2; margin: 8px 0;"><strong>Venue:</strong> Paman Event Centre, Along Yebumot Hotel, opposite INEC Office, Ilorin Kwara State</p>
            <p style="color: #FDFAF2; margin: 8px 0;"><strong>Dress Code:</strong> Chosen Aso ebi & Orange Cap (Male)</p>
          </div>
  ...
  console.log(
    `Scheduled email ${emailNum} completed: ${sent} sent, ${failed} failed`,
  );
  return { sent, failed };
}
